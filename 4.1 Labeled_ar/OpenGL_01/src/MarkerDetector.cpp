...
void MarkerDetector::processFrame(const cv::Mat imgOrigin, char flag)
{
	showFlag = flag;
    std::vector<Marker> markers;
	cv::namedWindow("Image", 2);
	cv::resizeWindow("Image", 1000, 562);
    findMarkers(imgOrigin, markers);
...
bool MarkerDetector::findMarkers(cv::Mat imgOrigin, std::vector<Marker>& detectedMarkers)
{
    cv::Mat bgraMat = imgOrigin.clone();

    // Convert the image to grayscale
    prepareImage(bgraMat, m_grayscaleImage);
	//cv::imshow("Image", m_grayscaleImage);
	//cv::waitKey(showFlag);
    // Make it binary
    performThreshold(m_grayscaleImage, m_thresholdImg);
	//cv::imshow("Image", m_thresholdImg);
	//cv::waitKey(showFlag);
    // Detect contours
	std::vector<cv::Vec4i> hierarchy;
	cv::Mat imgContour0 = cv::Mat::zeros(imgOrigin.size(), CV_8UC1);
	findContours(m_thresholdImg, m_contours, m_grayscaleImage.cols / 5);
	/*for (int i = 0; i < m_contours.size(); i++) {
		if (cv::contourArea(m_contours[i]) < 100) continue;
		for (int j = 0; j < m_contours[i].size(); j++) {
			drawContours(imgContour0, m_contours, i, cv::Scalar(255), 1, 8, hierarchy);
		}
	}*/
	//cv::imshow("Image", imgContour0);
	//cv::waitKey(showFlag);
	...
	for (Marker marker : detectedMarkers) {
		marker.drawContour(imgOrigin);
	}
	cv::imshow("Image", imgOrigin);
	cv::waitKey(showFlag);
...
{
    cv::threshold(grayscale, thresholdImg, 180, 255, cv::THRESH_BINARY_INV);
...
void MarkerDetector::estimatePosition(std::vector<Marker>& detectedMarkers)
{
    ...
    {					
		...
		// std::cout << camMatrix << std::endl << std::endl << distCoeff << std::endl;
